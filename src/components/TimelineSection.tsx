...
const moments = [
  { icon: Calendar, title: "Nosso primeiro encontro", text: "O dia em que tudo mudou pra sempre" },
  { icon: Heart, title: "Nosso primeiro beijo", text: "O momento que ficou gravado na minha alma" },
  { icon: MapPin, title: "Nossas aventuras", text: "Cada lugar ao seu lado vira o meu favorito" },
  { icon: Music, title: "Nossa música", text: "Toda vez que toca, eu penso em você" },
];
              ...
              <div className={`ml-16 md:ml-0 md:w-1/2 ${i % 2 === 0 ? "md:pr-12" : "md:pl-12"}`}>
                <div className="bg-background/80 backdrop-blur-sm rounded-2xl p-6 border border-border shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <h3 className="font-display font-semibold text-foreground">{moment.title}</h3>
                  </div>
                  <p className="text-muted-foreground font-body text-sm italic">{moment.text}</p>
                </div>
              </div>
            </motion.div>
          ))}
